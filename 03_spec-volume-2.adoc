...
Data Model document (see [link:#_dap4_chunked_data_representation[1.7]]).
...
[link:#Data_Model[Data Model, Persistent Representations, and Constraints
]] document.
...
link:#_content_negotiation_and_media_types[HTTP
...
Second, to support server-driven content negotiation (see the section titled
link:#_content_negotiation_and_media_types[HTTP
- Content Negotiation and Media Types] below) and
...
Response_ [link:#_dap4_dataset_services_response[2.3.1]].
...
"`application/octet-stream`". (See link:#_dap4_resource_roles_and_media_types[section
...
server (see section link:#Data_Model[1]).
...
expressions can be found in Volume 1 of the DAP4 specification (see section link:#Data_Model[1]).
...
specification (see section link:#Data_Model[1]).
...
the Dataset Metadata Response (an XML document described (see section link:#Data_Model[Volume 1]) of the
...
specification (see link:#Data_Model[[1]]).
...
specification (see link:#Data_Model[[1]]). The media type for the normative XML representation is:
...
link:#Data_Model[Volume 1], while the other responses are simple HTTP responses.
...
found in Section 8 of Volume 1 of the DAP4 specification (see link:#_constraints[[1.8]]).

*dap4.async* +
The asynchronous response behavior is described in detail in link:#l#netcdf-async[Section 5].
