...
For a dmr response the checksum is made visible to the client by the addition
of an Attribute to each top-level variable in the DMR. This attribute is named
...
The serialized data response indicates to the client that checksums are
...
=== Lexical Structure ===
Each chunk header is defined by 4 8 bit byte values. One 8-bit byte for *Chunk Type*, and three 8-bit bytes
for *Chunk Size*.
...
==== Chunk Size
Chunk Size ia expressed as a sequence of three 8-bit bytes, interpreted as an integer value in network byte order.
